...
# Start a completely new session - the agent won't know our name
    ...
    session_name="new-isolated-session",
)
...
# Expected: The agent won't know the name because this is a different session
    ...
    # session = await session_service.get_session(
    #     app_name=research_runner_compacting.app_name, user_id=USER_ID, session_id="state-demo-session"
    # )
    ...
    session_new = await session_service.get_session(
        app_name=research_runner_compacting.app_name, user_id=USER_ID, session_id="new-isolated-session"
    )

    print("New Session State:")
    print(session_new.state)
